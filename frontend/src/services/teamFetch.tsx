...
export const fetchTeamPlayerData = async (finalTeam: string) => {
    ...
    const qb: Player = {pos: "QB", name: data.qb[0], stats: data.qb.slice(3, 6), image: data.qb[6]};
    const rb: Player = {pos: "RB", name: data.rb[0], stats: data.rb.slice(1, 4), image: data.rb[5]};
    ...
    const def: Player = {pos: "DEF", name: finalTeam, stats: [data.def[1], data.def[3], data.def[4]]};
